...
app.get('/clientes/:id', (req, res) => {
    ...
    if (cliente) {
      res.json(cliente);  // Responde con el cliente si lo encuentra
    } else {
      res.status(404).json({ error: "cliente no encontrado" });  // Responde con un error en formato JSON
    }
  });
...
app.put('/clientes/:id_cliente', (req, res) => {
    ...
    // Si no se encuentra el cliente, responde con JSON
    if (clienteIndex === -1) {
        return res.status(404).json({ error: "Cliente no encontrado" });
    }

    // Si el cliente es encontrado, lo actualiza
    ...
    bd.clientes[clienteIndex] = actualizarCliente;
    writeDB(bd); // Llama a la función para guardar los cambios en la base de datos
